...
def buildWidget(proj, *args, **kwargs):
    ...
    if "loader_open" in state_defs:
        ...
        component_body += "      console.log('[LOADER DEBUG] *** WAITING 5 SECONDS - CHECK CONSOLE NOW ***');\n"
        component_body += "      console.log('[LOADER DEBUG] You have 5 seconds to see these messages!');\n"
        component_body += "      setTimeout(() => {\n"
        component_body += "        try {\n"
        component_body += "          console.log('[LOADER DEBUG] 5 seconds elapsed, proceeding with model.set...');\n"
        component_body += "          console.log('[LOADER DEBUG] Before model.set - loader_open:', model.get('loader_open'));\n"
        component_body += "          console.log('[LOADER DEBUG] Calling model.set(\\'loader_open\\', false)...');\n"
        component_body += "          const result = model.set('loader_open', false);\n"
        component_body += "          console.log('[LOADER DEBUG] model.set returned:', result);\n"
        component_body += "          console.log('[LOADER DEBUG] model.set completed successfully!');\n"
        component_body += "          console.log('[LOADER DEBUG] After model.set - loader_open:', model.get('loader_open'));\n"
        component_body += "        } catch (error) {\n"
        component_body += "          console.error('[LOADER DEBUG] *** ERROR CAUGHT ***:', error);\n"
        component_body += "          console.error('[LOADER DEBUG] Error message:', error.message);\n"
        component_body += "          console.error('[LOADER DEBUG] Error stack:', error.stack);\n"
        component_body += "          console.error('[LOADER DEBUG] Error type:', error.constructor.name);\n"
        component_body += "        }\n"
        component_body += "      }, 5000);\n"
        component_body += "    }, 100);\n"
